            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex">
                    ...
                    <button type="button" class="btn btn-secondary ms-2" data-bs-toggle="modal"
                        data-bs-target="#statusPengajuanModal">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $p->updated_at ? $p->updated_at->format('d-m-Y') : ($p->created_at ?
                                $p->created_at->format('d-m-Y') : '-') }}</td>
                            <td>{{ $p->keterangan ?? '-' }}</td>
                            <td>
                                <a href="{{ route('pendeta.pelayanan.pindah.show', $p->id) }}"
                                    class="btn btn-info btn-sm">Lihat</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted">Tidak ada permohonan pindah yang
                                dipublikasikan.</td>
                        </tr>
                        @endforelse
                    </tbody>
...
<div class="modal fade" id="statusPengajuanModal" tabindex="-1" aria-labelledby="statusPengajuanModalLabel"
    aria-hidden="true">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('pendeta.pelayanan.pindah.approve', $p->id) }}" method="POST"
                                        style="display:inline">
                                        @csrf
                                        <button class="btn btn-success btn-sm" type="submit">Setujui (kirim ke
                                            admin)</button>
                                    </form>

                                    <form id="modal-reject-form-{{ $p->id }}"
                                        action="{{ route('pendeta.pelayanan.pindah.reject', $p->id) }}" method="POST"
                                        style="display:inline">
                                        @csrf
                                        <input type="hidden" name="reason" value="" />
                                        <button type="button" class="btn btn-danger btn-sm"
                                            onclick="promptReject({{ $p->id }}, 'modal-reject-form-')">Tolak</button>
                                    </form>

                                    <a href="{{ route('pendeta.pelayanan.pindah.show', $p->id) }}"
                                        class="btn btn-info btn-sm ms-1">Lihat</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
